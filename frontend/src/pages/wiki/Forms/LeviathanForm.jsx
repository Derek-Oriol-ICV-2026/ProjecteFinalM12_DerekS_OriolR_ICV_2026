...
import './LeviathanForm.css'

export default function LeviathanForm({ leviathan, onSave, onClose, loading }) {
    const [formData, setFormData] = useState({ name: '', description: '', image_url: '', stats: { value: '' } })
    const [errors, setErrors] = useState({})

    useEffect(() => {
        if (leviathan) setFormData({
            name: leviathan.name || '',
            description: leviathan.description || '',
            image_url: leviathan.image_url || '',
            stats: { value: leviathan.stats?.value || '' }
        })
    }, [leviathan])
    ...
    const handleSubmit = (e) => {
        e.preventDefault()
        if (formData.name.trim()) onSave({ ...formData, type: 'leviathan' })
    }
        ...
        <div className="form-overlay">
            <div className="form-container">
                <div className="form-header">
                    <h2>{leviathan ? 'Editar Leviathan' : 'Crear Leviathan'}</h2>
                ...
                <form onSubmit={handleSubmit} className="form">
                    ...
                    <div className="form-buttons">
                        ...
                        <button type="submit" className="btn-save" disabled={loading}>{loading ? 'Guardando...' : (leviathan ? 'Actualizar' : 'Crear')}</button>
                    </div>
                </form>
            </div>
        </div>
